...
final class RuntimeEnvironmentController extends Notifier<AcpRuntimeState> {
  ...
  Future<RemoteSessionHandle> _openRemote(RemoteConnectionProfile profile) {
    ...
    if (connect == null) {
      throw StateError('Remote connection adapter was not injected');
    }
  ...
  Future<void> activateConnection(AcpConnection connection) async {
    ...
    final connect = _connectAcp;
    final bootstrap = connect == null
        ? const RuntimeBootstrap.failed(
            'ACP connection adapter was not injected',
          )
  ...
  Future<void> activateRemote(RemoteConnectionProfile profile) async {
    ...
    try {
      final handle = await _openRemote(profile);
