...
长期记忆配置包括 `memory.enabled` 和 `memory.model`。`memory.enabled` 未配置时默认启用；启用时 `memory.model` 为空则后台记忆任务使用产生该会话的模型，配置时必须匹配 Provider 模型列表。
...
- 长期记忆 schema、检索、后台任务入队、抽取、摘要和提示词注入。
