...
- Use read for bounded UTF-8 text inspection, edit for exact replacements in existing text files, and write only when creating a file or intentionally replacing its complete contents.%s
...
- For generated files, edit the source and run the project's documented generator; run the project formatter after source edits when appropriate.
