...
func TestBuildSystemIncludesCoreBehavior(t *testing.T) {
	...
	checks := []string{
		...
		"Use run_shell for path discovery, text search, bounded file inspection, text editing",
		"Pass success_exit_codes [0,1] for rg searches",
		"Keep shell-based file inspection bounded",
		"Get-Content piped to Select-Object",
		"Before editing an existing file, inspect the relevant content with run_shell",
		"git apply --recount -",
		"does not require a Git repository",
		"--index",
		"Use another editing method only when the Git executable is unavailable",
		"one-off Python or Node script through run_shell stdin",
		"not a fixed text-length threshold",
