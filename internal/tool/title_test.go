...
func TestDisplayTitleUsesPrimaryArgument(t *testing.T) {
	tests := []struct {
		...
		{name: "apply patch", call: model.ToolCall{Name: "apply_patch", Arguments: `{"patch":"--- a/a.txt\n+++ b/a.txt"}`}, want: "Patch: --- a/a.txt"},
		{name: "glob", call: model.ToolCall{Name: "glob", Arguments: `{"pattern":"**/*.go"}`}, want: "Glob: **/*.go"},
		{name: "grep", call: model.ToolCall{Name: "grep", Arguments: `{"pattern":"Tool:"}`}, want: "Grep: Tool:"},
