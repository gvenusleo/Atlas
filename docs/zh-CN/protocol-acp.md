...
- shell 过程输出使用标准 `tool_call_update` 文本内容，状态为 `in_progress`。
  每个快照替换之前的内容，最终结果再次替换并标记 `completed` 或 `failed`。
  即使执行失败或回放历史，`rawOutput` 仍保留捕获文本和最终 metadata；无需终端扩展。
- ACP 客户端在部分更新省略工具内容时保留原内容，包括仅更新完成状态和历史回放；显式空内容会清空原内容。
